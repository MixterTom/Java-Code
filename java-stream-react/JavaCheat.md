...
In Java, primitive data types serve as the foundation for manipulating data. They are the most basic types of data that the Java programming language uses. Java has 8 primitive data types: byte, short, int, long, float, double, char, and boolean.

**Kiến thức cơ bản về bit, byte và Data Types:**
- **bit**: Là đơn vị lưu trữ thông tin nhỏ nhất trong máy tính, có giá trị là `0` hoặc `1`.
- **1 byte** = 8 bits.
- **int**: 1 `int` = 4 bytes = 32 bits. Khoảng giá trị từ -2^31 đến 2^31-1 (tức là từ -2,147,483,648 đến 2,147,483,647).
- **byte**: 1 byte = 8 bits. Khoảng giá trị từ -128 đến 127.
- **short**: 2 bytes = 16 bits. Khoảng giá trị từ -32,768 đến 32,767.
- **long**: 8 bytes = 64 bits. Khoảng giá trị từ -2^63 đến 2^63-1.
- **float**: 4 bytes = 32 bits.
- **double**: 8 bytes = 64 bits.
- **char**: 2 bytes = 16 bits. Khoảng giá trị từ 0 đến 65,535 (kí tự Unicode).
- **boolean**: Kích thước không được định nghĩa chính xác, phụ thuộc vào JVM, thường hoạt động giống như 1 bit.

**Hệ cơ số (Number Systems) và Chuyển đổi (Conversions):**
Trong Java, bạn có thể biểu diễn số nguyên `int` theo nhiều hệ cơ số khác nhau:
- **Thập phân (Decimal - Cơ số 10)**: Giá trị mặc định. *(Ví dụ: `int a = 15;`)*
- **Nhị phân (Binary - Cơ số 2)**: Bắt đầu bằng `0b` hoặc `0B`. *(Ví dụ: `int b = 0b1111;` -> 15)*
- **Bát phân (Octal - Cơ số 8)**: Bắt đầu bằng `0`. *(Ví dụ: `int c = 017;` -> 15)*
- **Thập lục phân (Hexadecimal - Cơ số 16)**: Bắt đầu bằng `0x` hoặc `0X`. *(Ví dụ: `int d = 0xF;` -> 15)*

*Các phương thức chuyển đổi cơ bản (Sử dụng class `Integer`):*
```java
int number = 15;

// 1. Từ int (Thập phân) sang Chuỗi Nhị phân, Bát phân, Thập lục phân
String binaryStr = Integer.toBinaryString(number); // Kết quả: "1111"
String octalStr = Integer.toOctalString(number);   // Kết quả: "17"
String hexStr = Integer.toHexString(number);       // Kết quả: "f"

// 2. Từ Chuỗi Nhị phân, Bát phân, Thập lục phân sang int (Thập phân)
int fromBinary = Integer.parseInt("1111", 2);      // Kết quả: 15
int fromOctal = Integer.parseInt("17", 8);         // Kết quả: 15
int fromHex = Integer.parseInt("F", 16);           // Kết quả: 15
```
...
25. Object-Oriented Programming (OOP) in Java
Java is inherently an Object-Oriented Programming (OOP) language. The four main pillars of OOP in Java are:

1. **Encapsulation (Đóng gói)**
   - It is the mechanism of wrapping the data (variables) and code acting on the data (methods) together as a single unit.
   - It is typically achieved by making class variables `private` and providing `public` getter and setter methods.

2. **Inheritance (Kế thừa)**
   - *(See section 14. Java Inheritance)*
   - It allows a new class to inherit properties and methods from an existing class, promoting code reusability.

3. **Polymorphism (Đa hình)**
   - *(See section 13. Java Polymorphism)*
   - The ability of a variable, function, or object to take on multiple forms. It is implemented via Method Overloading (Compile-time) and Method Overriding (Run-time).

4. **Abstraction (Trừu tượng)**
   - It is the process of hiding the implementation details and showing only functionality to the user.
   - Achieved using abstract classes (`abstract` keyword) and interfaces (`interface`).

**Ví dụ cơ bản về OOP (OOP Demo):**
```java
// Abstraction
